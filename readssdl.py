...
if len(sys.argv) < 2:
	if debug:
		...
		print("Usage: python readssdl.py [path]<filename>")
		sys.exit(0)
else:
	filename = sys.argv[1]
...
'''
	...
	Constants (from Sddl.h):
		ace_type:
			...
			"XA" 	SDDL_CALLBACK_ACCESS_ALLOWED			ACCESS_ALLOWED_CALLBACK_ACE_TYPE	0x09	??? this is identical ???
			...
			"ZA" 	SDDL_CALLBACK_OBJECT_ACCESS_ALLOWED		ACCESS_ALLOWED_CALLBACK_ACE_TYPE	0x09	??? this is identical ???
		ace_flags:
...
# -------------------------------------- SSDL and ACE Constants (from Sddl.h):
...
def show_sddl(filename): 
	...
	for i in range(len(sddls)):
		...
		if i == 0:
			...
			type, flags, rights, guid, iguid, sid = ssdl.split(';')
			print("  {:20} :: ".format(ssdl), end='')
			if len(sid) > 0:
				if sid in SDDL_SIDS: 
					print('{:30}'.format(SDDL_SIDS[sid]), end='')
			print('{:25} '.format(SDDL_TYPE[type][0]), end='')
			if len(flags) > 0:
				fl = SDDL_FLAGS[flags][0]
			else:
				fl = ''
			print('{:20} '.format(fl), end='')
			if len(rights) > 0:
